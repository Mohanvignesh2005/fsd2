...
export const useAuthstore = create((set, get) => ({
  ...
  try {
    ...
    // Check if backend returned user
    ...
    if (res.status === 200 && user) {
      ...
      // If for some reason status is 200 but no user
      toast?.error(message);
    }
  } catch (error) {
    console.log("Login Error:", error);

    // Safely get backend error message
    const errorMessage =
      error?.response?.data?.message || // backend message
      error?.response?.data?.error ||   // fallback backend error
      error?.message ||                 // Axios or JS error
